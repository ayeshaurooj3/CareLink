...
def handle_result(model, template, crop=False, resize=(224, 224)):
    if request.method == 'POST':
        file = request.files['file']
        if file and allowed_file(file.filename):
            filename = secure_filename(file.filename)
            ...
            if crop:
                img = crop_imgs([img])[0]  # Assuming crop_imgs is defined
            if resize:
                img = cv2.resize(img, resize)
            img = img.reshape(1, *resize, 3) / 255.0
            pred = model.predict(img)
            pred = int(pred[0] > 0.5)  # Binary classification
            return render_template(template, filename=filename, r=pred)
        else:
            flash('Allowed image types are - png, jpg, jpeg')
            return redirect(request.url)
